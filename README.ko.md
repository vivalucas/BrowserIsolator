...
- **환경 정보 표시**: 상태, 폴더, 환경 차이 모드 실행 시의 디버깅 포트, 디스크 사용량, 마지막 사용 시간
- **이름과 메모**: 계정이나 용도에 맞게 환경 이름과 짧은 메모 지정
- **호환성 우선**: 기본값은 profile 데이터만 분리하며 디버깅 포트나 페이지 스크립트 주입을 사용하지 않습니다
- **선택적 지문 차별화**: 설정에서 환경별로 켤 수 있습니다. 켠 환경은 다음 시작 때 `navigator.hardwareConcurrency`와 `navigator.deviceMemory`를 주입합니다
...
- **환경 차이 모드**: 설정에서 환경별로 켭니다. 실행 중인 환경은 변경할 수 없으며, 닫은 뒤 바꾸고 다음 시작 때 적용됩니다
...
기본값에서는 아무것도 주입하지 않습니다. BrowserIsolator는 호환 모드를 우선하며 로컬 profile 데이터만 분리합니다.

설정에서 환경 차이 모드를 켠 환경은 다음 시작 때 Chrome DevTools Protocol로 `navigator.hardwareConcurrency`와 `navigator.deviceMemory` 값을 환경별로 설정합니다. 현재 page target을 주기적으로 동기화하므로 새로 연 탭에도 주입됩니다. 완전한 기기 시뮬레이션은 아닙니다.
