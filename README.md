...
- **兼容优先**：默认只做 profile 数据隔离，不启用调试端口或页面脚本注入，尽量接近普通 Chrome 的访问行为
- **可选环境差异**：可在设置中按环境开启轻量差异模式；开启后下次启动会注入稳定的 `navigator.hardwareConcurrency` 和 `navigator.deviceMemory` 值
...
- **右侧详情栏**：显示选中环境的 profile 路径、调试端口（仅环境差异模式运行时）、错误信息、操作按钮和高级详情
...
- **查看详情**：选中环境，在右侧详情栏查看 profile 路径、调试端口（仅环境差异模式运行时）、错误信息、操作和高级详情
...
- **环境差异模式**：在设置中按环境开启；运行中的环境不能切换，需要关闭后修改，下次启动生效
...
默认情况下什么都不注入。BrowserIsolator 优先使用兼容模式，只隔离本地 profile 数据，不开启调试端口。

如果你在设置中为某个环境开启“环境差异模式”，该环境下次启动时会通过 Chrome DevTools Protocol 给页面注入脚本，为不同环境设置不同的 CPU 核心数和内存值：
