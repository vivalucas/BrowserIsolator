...
- **快速启动和关闭**：在主窗口或菜单栏启动、关闭单个环境；也可以选中左侧环境行后再次点击启动，或一次关闭全部环境
- **专业管理界面**：左侧环境列表用于快速扫描，右侧详情栏展示路径、模式、错误、操作和高级信息
- **界面状态记忆**：主窗口大小、位置和左右分栏宽度会在下次打开时恢复
- **自定义命名和备注**：新增环境后可以直接命名，也可以之后重命名或添加备注，方便对应不同账号、客户或用途
- **安全删除**：删除环境需要输入环境名称确认，数据会先移到废纸篓
- **基础模式优先**：默认只做 profile 数据隔离，不启用调试端口或页面脚本注入，尽量接近普通 Chrome 的访问行为
- **采集与差异模式分离**：采集模式仅开放绑定 `127.0.0.1` 的本机 CDP 端口；差异模式额外注入稳定的 `navigator.hardwareConcurrency` 和 `navigator.deviceMemory`，可按环境独立组合
- **外部链接转发**：可以把 BrowserIsolator 设为系统默认浏览器，并指定外部链接默认进入哪个环境；目标不可用时会弹出提示，避免静默丢失链接
- **自动浏览器安装**：首次运行时自动下载官方 Google Chrome 到应用数据目录
- **设置面板**：查看 Chrome 状态和版本、管理采集与差异模式、打开数据目录、配置外部链接、切换语言和外观，以及查看更新与反馈入口
- **三级配置恢复**：保存时保留 `.bak`；主配置损坏时优先恢复备份，再按磁盘中的 `Profiles/pN` 重建环境，并保留损坏副本用于排查
...
- **右侧详情栏**：基础信息显示 profile 路径、运行模式、错误信息和操作按钮；调试端口仅在高级信息中按需显示

常用操作：

- **启动环境**：点击环境行右侧的启动按钮、选中左侧环境行后再次点击，或在右侧详情栏点击“启动”
...
- **查看详情**：选中环境，在右侧详情栏查看 profile 路径、运行模式、错误信息、操作和高级详情
- **删除环境**：右键未运行环境或在详情栏点击“删除”；输入环境名称确认后，数据会移到废纸篓
- **外部链接**：在设置里打开“外部链接”，选择外部链接要打开到哪个环境，并可将 BrowserIsolator 设为默认浏览器。如果浏览器环境还没准备好，或当前没有可用环境，应用会提示并允许复制链接；没有可用环境时也可以直接打开主面板
- **设置**：点击工具栏齿轮，查看 Chrome 状态、打开数据目录、复制路径、重新下载 Chrome、配置外部链接、切换语言和外观模式，或在“关于与支持”中检查更新、查看发布页、查看作者联系方式和提交反馈。重新下载 Chrome 时，所有环境必须处于非启动、非运行、非关闭中状态
- **差异模式**：在设置中按环境开启；运行中的环境不能切换，需要关闭后修改，下次启动生效
...
如果 `config.json` 损坏或无法读取，应用会提示问题、加载默认配置，并在可能时把原文件备份为 `config.corrupt-<时间戳>.json`，方便之后排查或手动恢复。
...
默认情况下什么都不注入。BrowserIsolator 优先使用基础模式，只隔离本地 profile 数据，不开启调试端口。需要本机采集工具连接时，可单独为环境开启采集模式；只有确实需要 navigator 差异时才开启差异模式。调试端口优先使用 `41000 + 环境编号`，冲突时自动尝试后续端口。

如果你在设置中为某个环境开启“差异模式”，该环境下次启动时会通过 Chrome DevTools Protocol 给页面注入脚本，为不同环境设置不同的 CPU 核心数和内存值：
...
运行中的环境不能切换这个模式，因为它影响 Chrome 启动参数和是否启用 CDP。请先关闭环境，修改设置后再启动。
...
不会。当前下载的 Chrome 会一直使用。如果需要更新，优先进入 **设置 -> 重新下载 Chrome**。重新下载时，所有环境都需要先关闭，且不能处于启动中或关闭中状态。
...
如果浏览器环境还没准备好，或当前没有可用环境，BrowserIsolator 不会静默吞掉链接，而是会弹出提示并提供复制链接按钮；没有可用环境时也可以直接打开主面板。
