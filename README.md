...
- **可选差异模式**：可在设置中按环境开启；开启后下次启动会注入稳定的 `navigator.hardwareConcurrency` 和 `navigator.deviceMemory` 值
...
- **配置恢复提示**：配置文件损坏或无法读取时会加载默认配置，并尽量保留一份损坏文件备份
...
默认情况下什么都不注入。BrowserIsolator 优先使用基础模式，只隔离本地 profile 数据，不开启调试端口。
