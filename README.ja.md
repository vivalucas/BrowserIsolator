...
- **ワンクリック操作**：メインパネルまたはメニューバーから起動・終了できます。選択済みの環境行をもう一度クリックして起動でき、終了時は Chrome の終了を待ちます
- **環境情報表示**：左側の一覧で状態、容量、最終使用日時を確認し、右側の詳細欄で profile パス、実行モード、エラー、操作、高度な情報を確認できます。デバッグポートは必要な場合のみ詳細情報に表示
- **レイアウトの記憶**：メインウィンドウのサイズ、位置、サイドバー幅は次回起動時に復元されます
- **名前とメモ**：環境に分かりやすい名前や短いメモを付けられます
- **安全な削除**：削除時は環境名の入力が必要で、データはゴミ箱へ移動します
...
- **外部リンク**：BrowserIsolator を既定のブラウザにし、他のアプリから開いたリンクを指定した環境に送れます。開き先が使えない場合はリンクを失わないよう確認画面を表示します
- **ブラウザ自動インストール**：初回起動時に公式 Google Chrome を自動ダウンロード
- **設定パネル**：Chrome の状態とバージョン、データフォルダ、パスコピー、Chrome の再ダウンロード、外部リンク、言語、外観、高度な詳細表示、更新、作者連絡先、フィードバックを確認できます
- **設定復旧**：`config.json` が壊れている、または読めない場合は既定設定で起動し、可能な場合は壊れたファイルをバックアップします
- **ローカル保存**：設定、Chrome、環境データはすべて Mac 内に保存し、ユーザーデータをアップロードまたは収集しません
...
- **起動**：環境行の「起動」をクリック、選択済みの環境行をもう一度クリック、または右側の詳細欄から起動します。失敗した場合はエラーが表示されます
...
- **外部リンク**：**設定 -> 外部リンク** で開き先の環境を選び、BrowserIsolator を既定のブラウザに設定できます。環境がまだ準備できていない、または利用可能な環境がない場合はリンクをコピーでき、利用可能な環境がない場合はメインパネルも開けます
- **設定**：Chrome の状態、データフォルダ、パスコピー、Chrome の再ダウンロード、外観、言語、更新、作者情報、フィードバックを確認できます。Chrome の再ダウンロードは、起動中・実行中・終了中の環境がない場合だけ使えます
...
`config.json` が壊れている、または読めない場合、BrowserIsolator は警告を表示して既定設定を読み込み、可能な場合は元のファイルを `config.corrupt-<timestamp>.json` として保存します。
...
されません。更新したい場合は、まず **設定 -> Chrome を再ダウンロード** を使ってください。すべての環境が停止していて、起動中または終了中でない必要があります。手動で更新する場合は `~/Library/Application Support/BrowserIsolator/Chromium/` を削除してから再起動してください。
...
**設定 -> 外部リンク** で開き先の環境を選び、「既定のブラウザにする」をクリックします。メール、チャット、メモなどから開いた http/https リンクが選択した環境に送られます。環境がまだ準備できていない、または利用可能な環境がない場合は、リンクをコピーできる確認画面を表示します。利用可能な環境がない場合はメインパネルも開けます。
