...
- **環境情報表示**：状態、フォルダ、環境差分モード実行時のデバッグポート、容量、最終使用日時を表示
- **名前とメモ**：環境に分かりやすい名前や短いメモを付けられます
- **互換性優先**：既定では profile データだけを分離し、デバッグポートやページスクリプト注入は使いません
- **任意の指紋差異化**：設定で環境ごとに有効化できます。有効な環境は次回起動時に `navigator.hardwareConcurrency` と `navigator.deviceMemory` を注入します
...
- **環境差分モード**：設定で環境ごとに有効化できます。実行中の環境は変更できないため、閉じてから変更し、次回起動で反映します
...
既定では何も注入しません。BrowserIsolator は互換モードを優先し、ローカル profile データだけを分離します。

設定で環境差分モードを有効にした環境は、次回起動時に Chrome DevTools Protocol で `navigator.hardwareConcurrency` と `navigator.deviceMemory` を環境ごとに設定します。現在の page target を定期的に同期するため、新しく開いたタブにも注入されます。完全なデバイス模擬ではありません。
